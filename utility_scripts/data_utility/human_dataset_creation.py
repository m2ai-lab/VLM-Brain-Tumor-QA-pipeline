...
def main(args):
    ...
    if os.path.exists(args.scan_mapping_path):
        ...
        scan_mapping = pd.DataFrame(columns=['Deidentified_Accession_Number', 'sequence', 'image_path'])
    ...
    for accession_num, count in freq_series.items():
        ...
        rows = dataset[dataset['Deidentified_Accession_Number'] == accession_num]
        if len(rows) > total:
        ...
        seq_paths = scan_mapping[scan_mapping['Deidentified_Accession_Number'] == str(accession_num)]
        if seq_paths.empty:
            seq_paths = scan_mapping[scan_mapping['Deidentified_Accession_Number'] == accession_num]
