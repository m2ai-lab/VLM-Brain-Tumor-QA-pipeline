...
# Specific filenames for this variant
MONTAGE_FILENAMES = [
    "axial_slices_montage.png",
    "coronal_slices_montage.png",
    "sagittal_slices_montage.png"
]
...
Example Response:
{
  "reasoning": "The multi-axis montages show significant mass effect and central necrosis across all planes, confirming aggressive growth.",
  "answer": "2) High Grade"
}
"""
...
class MedResponse(BaseModel):
    reasoning: str = Field(description="Step-by-step clinical observation of the MRI montages.")
    answer: str = Field(description="The final choice selected from the options.")

def process_montages(image_dir: str) -> list[Image.Image]:
    """Loads Axial/Coronal/Sagittal montages from a directory."""
    images = []
    for fname in MONTAGE_FILENAMES:
        img_path = path.join(image_dir, fname)
...
def _build_prompt(question: str) -> str:
    return (
        "Instruction: You are a neuroradiologist. Analyze the provided multi-axis "
        "MRI montages (Axial, Coronal, Sagittal) and provide a structured JSON response.\n"
        f"{FEW_SHOT_EXAMPLE}"
        "---\n"
...
def run_batch(model, processor, batch_rows: list[dict], base_image_dir: str) -> list[dict]:
    ...
    for i, row in enumerate(batch_rows):
        ...
        images = process_montages(patient_image_dir)
        if not images:
            results[i] = {"reasoning": "Error: No montages found.", "answer": "Error"}
            continue
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MedGemma Multi-Axis Montage Inference")
    parser.add_argument('--qa_path', type=str, default=_cfg.get("qa_path"))
    parser.add_argument('--output_path', type=str, default=_cfg.get("output_base", "") + "/MedGemma1.5/contrast_montage_results.csv")
